...
const MapGame = (() => {
    ...
    function init() {
        ...
        completedRegions = data.player.completedRegions || [];
        renderMap();
        updateProgress();

        const az = document.getElementById('map-char-azzam');
        const jd = document.getElementById('map-char-jadel');
        ...
        // Play Map Intro Dialogue
        const introAudio = HawdajAudio.getRandomAudioPath('jadel', 'intro_map');
        HawdajDialogue.showIntro(
            "أهلاً بك في خريطة المملكة! استكشف المناطق واضغط عليها لتتعرف على معالمها وتجاوب على أسئلتها.",
            introAudio,
            () => {
    ...
    <svg viewBox="0 0 730 600" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto;">
      ...
      <text x="291.6421800000001" y="498.76040999999975" text-anchor="middle" fill="rgba(255,255,255,0.9)" font-size="12" font-weight="700" pointer-events="none" style="text-shadow: 0px 1px 3px rgba(0,0,0,0.8);">عسير</text>
        ...
        container.querySelectorAll('path[data-region]').forEach(path => {
            const regionId = path.dataset.region;

            path.addEventListener('click', () => {
                HawdajAudio.SFX.mapClick();
            ...
            path.addEventListener('mousemove', (e) => {
                if (!tooltip) return;
                const data = HawdajData.get();
                const regionName = data.regions[regionId]?.name || regionId;
    ...
    function showRegionInfo(regionId) {
        ...
        document.getElementById('region-capital').textContent = `العاصمة: ${region.capital} `;
        document.getElementById('region-facts').textContent = region.facts;

        // Clothing Info
        const clothingContainer = document.getElementById('region-clothing-container');
        const clothingImg = document.getElementById('region-clothing-display-img');
        const clothingText = document.getElementById('region-clothing-display-text');

        if (region.clothingImage || region.clothingText) {
            clothingContainer.style.display = 'block';
            if (region.clothingImage) {
                clothingImg.src = region.clothingImage;
                clothingImg.style.display = 'block';
            } else {
                clothingImg.style.display = 'none';
            }
            if (region.clothingText) {
                clothingText.textContent = region.clothingText;
                clothingText.style.display = 'block';
            } else {
                clothingText.style.display = 'none';
            }
        } else {
        ...
        // Render questions
        const qContainer = document.getElementById('region-questions');
        qContainer.innerHTML = '';

        const questions = region.questions || [];
        questions.forEach((q, idx) => {
            const qDiv = document.createElement('div');
            qDiv.style.cssText = 'margin-bottom:16px; text-align:center;';

            const qText = document.createElement('p');
            qText.textContent = `${idx + 1}. ${q.question} `;
            qText.style.cssText = 'font-weight:700; margin-bottom:8px; color:var(--text-primary);';
            ...
            q.options.forEach((opt, optIdx) => {
                ...
                btn.style.cssText = 'min-height:40px; font-size:var(--fs-sm); animation:none;';
                btn.textContent = opt;
                btn.addEventListener('click', () => {
                    if (btn.classList.contains('disabled')) return;

                    // Disable all options for this question
                    optGrid.querySelectorAll('.quiz-option').forEach(b => b.classList.add('disabled'));

                    if (optIdx === q.correct) {
                        btn.classList.add('correct');
                        HawdajAudio.SFX.correct();
                        HawdajEffects.sparkle(btn.getBoundingClientRect().left + btn.offsetWidth / 2, btn.getBoundingClientRect().top);
                        if (window.HawdajCelebration) HawdajCelebration.show('map');
                    } else {
                        if (window.PowerupInventory && PowerupInventory.hasShield()) {
                            ...
                            optGrid.querySelectorAll('.quiz-option').forEach(b => {
                        ...
                        // Highlight correct
                        optGrid.querySelectorAll('.quiz-option')[q.correct].classList.add('highlight-correct');
                        if (window.HawdajCelebration) HawdajCelebration.show('wrong');
                    }
                });
    ...
    function closeRegionModal(regionId) {
        document.getElementById('region-modal').classList.remove('active');

        // Mark as completed first time
        if (regionId && !completedRegions.includes(regionId)) {
            const data = HawdajData.get();
            // Check if all questions were answered (simplified: just viewing counts)
            completedRegions.push(regionId);
            data.player.completedRegions = completedRegions;
            let mapPts = data.points.region;
            if (window.PowerupInventory) {
                mapPts = Math.floor(mapPts * PowerupInventory.getScoreMultiplier());
            }
            HawdajData.addScore(mapPts, 'map');
            HawdajData.save();

            // Daily task trigger
            if (window.completeDailyTask) {
                window.completeDailyTask('map_explore');
            }

            renderMap();
            updateProgress();
            HawdajApp.showToast(`🎉 اكتشفت ${data.regions[regionId]?.name} ! +${data.points.region} نقطة`, 'success');
        }
    }

    function updateProgress() {
        const total = 13;
        const done = completedRegions.length;
        const text = document.getElementById('map-progress-text');
        const fill = document.getElementById('map-progress-fill');
        if (text) text.textContent = `${done}/${total} منطقة`;
        ...
        if (done === total) {
            setTimeout(() => {
                ...
                HawdajApp.showToast('🏆 اكتشفت جميع مناطق السعودية!', 'success');
            }, 500);
        }
    }

    // Track which region was last clicked for closing
    let lastClickedRegion = null;

    function setLastClicked(regionId) {
        lastClickedRegion = regionId;
    }

    function getLastClicked() {
        return lastClickedRegion;
    }

    // Hint functionality for powerups
    function useHint() {
        const data = HawdajData.get();
        const uncompleted = Object.keys(data.regions).filter(id => !completedRegions.includes(id));
        if (uncompleted.length === 0) {
            HawdajApp.showToast('اكتشفت جميع المناطق بالفعل!', 'info');
            return;
        }

        // Pick random uncompleted region
        const hintRegion = uncompleted[Math.floor(Math.random() * uncompleted.length)];
        const path = document.getElementById(hintRegion);
        ...
        if (path) {
            ...
            path.style.fill = '#FFD700'; // Gold flash
            path.style.filter = 'drop-shadow(0 0 10px rgba(255, 215, 0, 0.8))';

            let p = 0;
            const ival = setInterval(() => {
                p++;
                path.style.transform = p % 2 === 0 ? 'scale(1.03)' : 'scale(1)';
                if (p > 5) {
                    clearInterval(ival);
            ...
            HawdajAudio.SFX.correct();
            HawdajApp.showToast(`💡 تلميح سحري: توجه إلى ${data.regions[hintRegion]?.name || 'منطقة جديدة'}`, 'success');
        }
    }

    return { init, renderMap, closeRegionModal, showRegionInfo, setLastClicked, getLastClicked, useHint };
})();
